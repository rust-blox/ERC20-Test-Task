...
contract Sextillion is IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 internal _totalSupply = 73000000000000000000000 *10**18; 
    ...
    function transfer(address recipient, uint256 amount) public nonReentrant override virtual returns (bool) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public nonReentrant override  virtual returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(amount <= _maxTxAmount, "Error about _maxTx.");
        require(balanceOf(recipient) + amount <= _maxWalletSize, "Error about maxWallet");
    ...
    function BuycollectFee(address account, uint256 amount) private returns (uint256) {
        ...
        if(_marketingFee > 0){
            transferAmount = transferAmount.sub(_marketingFee);
             _balances[_marketingWallet] = _balances[_marketingWallet].add(_marketingFee);
            _marketingTotal = _marketingTotal.add(_marketingFee);
            emit Transfer(account,_marketingWallet,_marketingFee);
        }
    ...
    function set_marketingWallet(address _Address) public onlyOwner {
        require(_Address != _marketingWallet);
